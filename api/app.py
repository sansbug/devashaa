...
@app.post("/api/matrix/backtest")
def matrix_backtest_endpoint():
    """Calibrate the projection against the user's own logged life events. See
    matrix.backtest. `events` = [{date:'YYYY-MM', key:<theme>, polarity:+1|-1}]."""
    body = request.get_json(silent=True) or {}
    missing = [f for f in ("date", "time", "latitude", "longitude") if body.get(f) in (None, "")]
    if missing:
        return jsonify({"error": f"Missing required field(s): {', '.join(missing)}"}), 400
    ...
    try:
        chart = compute_chart(local_dt=local_dt, latitude=lat, longitude=lon, tz_name=tz_name)
        out = matrix.backtest(chart, matrix.build(chart), events[:60])
    except Exception as e:  # noqa: BLE001
...
@app.post("/api/matrix/lifearc")
def matrix_lifearc_endpoint():
    """The life arc from birth — yearly values for six life-facets, the mahādaśā
    ribbon and the turning-point yogas. On demand (walks the whole life). See
    matrix.lifearc."""
    body = request.get_json(silent=True) or {}
    missing = [f for f in ("date", "time", "latitude", "longitude") if body.get(f) in (None, "")]
    if missing:
